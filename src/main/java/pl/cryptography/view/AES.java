...
//////////////////////////////// GENEROWANIE KLUCZA //////////////////////////////////
    ...
    public byte[][] expandKey(byte[] key){
        ...
        //zaczynac od poczatku calej tablicy tylko od miejsca gdzie sie skonczyl podany klucz
        //
        //Powtarzamy ten proces az do momentu gdy osiagniemy totalNumOfWords czyli
        //zapelnimy tablice klucza (temp)
        while(i < totalNumOfWords){
            //jako ze temp to tablica tablic to bierzemy po kolei kazda z nich
            //bierzemy i-1 bo rzeby wytworzyc i-ty wiersz slow potrzebujemy poprzedniego wiersza
            row = temp[i-1];

            if (i%numOfWords == 0){
                //wiersz ktory jest podzielny przez liczbe slow w kluczu (dla AES-128 co czwarty)
                //wykonyjemy funkcje g (nie wiem skąd taka nazwa ale no)
                ...
                //bo AES-256 ma klucz z 8 slowami a nie 4 lub 6 jak pozostale dwa
                //W kazdym razie co czwarte slowo jest poddawane substytucji bajtow
                //Wyjatek stanowia slowa podzielne przez 8 bo one normalnie ida do
                //funkcji g
                //czyli deFakto nie co 4 tylko tez co ósme słowo wpada w ten warunek
                row = subRow(row);
            }
    ...
    public byte[] subRow(byte[] row){
        ...
        for(int i = 0; i < dimensions; i++){
            ...
            //a nie ujemna. Bez tego moglo by dojsc do odczytania row[i] jako wartosci ujemnej
            //a w konsekwencji wyjatku bo index nie moze byc ujemny
            temp[i] = (byte) sbox[(row[i]&0xFF)];
        }
