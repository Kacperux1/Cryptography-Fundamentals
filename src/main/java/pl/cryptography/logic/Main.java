package pl.cryptography.logic;

public class Main {
    private static final int[] sbox = {
            
    };
    public static void main(String[] args) {
        byte[] testRow = {(byte) 0x19, (byte) 0xa0, (byte) 0x9a, (byte) 0xe9};
        for (byte b : testRow) {
            int index = b & 0xFF;  // Przemiana bajtu na zakres 0-255
            System.out.println(String.format("Value: 0x%02X -> Substituted: 0x%02X", b, sbox[index]));
        }

    }
}
